...
| **v3 PRODUCTIZATION** (2026-07-10) — the PRD loop realized on the k4kspec core (ADR-022) | Commits `271990d..` build the full propose/review/sign/certify product as files+CLI in `k4kspec/`: laws in the surface language (per-case `law` statements; 4 law built-ins as round-tripping surface files, emit byte-identical); `<name>.k4k/` ledger (signatures = version history, proposals, promoted certificates, last-failure); BLAKE256 signatures pin exact bytes + record under-spec acknowledgment (exit 4 without `--ack-underspec`) + tier waivers (laws only, single choke point `apply_waivers`, certificate MUST disclose — tamper-tested); certify GATES on a valid signature (exit 3; `--unsigned` = stamped dev run); computed certificate scope table; propose/revise/propose-fix retry-gated with a MONOTONIC decision journal + deterministic stubs (whole loop agent-free-testable); Kalgebra embedded in the binary (works from any cwd). **Fresh-agent audit: RED→fixed** — one read-twice TOCTOU in the gate (hash of read #1, parse of read #2), closed read-once, strace-verified 2→1 opens; every other attack mitigated (L24). **Live acceptance demo GREEN**: `propose grepclone "certified grep -F clone"` → signed-quality draft attempt 1 (boundary examples incl. empty needle + no-trailing-newline; decision-focused review artifacts; split-rule-respecting hints) → sign v1 → live proof attempt 1, 60/60 cross-check, certificate promoted → forced failure → live `propose-fix` (split the failing corner, honest cost analysis) → live `revise` ("add a count line") invented **7 interlocking laws** incl. anti-smuggling + canonicality when the vocabulary lacked int→string, monotone decisions held; the draft's bool-valued laws don't elaborate in v1 (`starts_with` unsupported) but the honesty chain contains it: check flags the un-oracle-checkable examples (revise exit 1) and sign refuses failing drafts. **v2 watcher tree: superseded, untouched — retirement is the named next pass.** |

...
**Default:** unset → `Backend_stub` and (when configured) `Verifier_stub` are used; `Backend_claude` and `Verifier_dune_ocaml` are still used for any test that explicitly requests a real verifier (e.g. `S1_echo_first_run_e2e` always uses real `dune`).
